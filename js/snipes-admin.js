(function(uri, port){
  ...
  s.onload = function() {
    ...
    socket.on('move', function (client) {
      ...
      $el.css({ 
        left: (client.x - frame.pageXOffset + avg(admin.w, $el.data('data').w) - 5) + 'px',
    ...
    socket.on('click', function (client) {
      ...
      $('#pulse_' + client.id)
        .css({ 
          left: (client.x - frame.pageXOffset + centerage - 5) + 'px',
          top:  (client.y - frame.pageYOffset - 4) + 'px'  
        })
        .animate({ 
          left: (client.x - frame.pageXOffset + centerage - 21) + 'px', 
  ...
  window.addEventListener('load', function () {
    ...
    frame.onscroll = function () {
      ...
      $('div[id^="cursor_"]').each(function () {
        ...
        $(this).css({ 
          left: (client.x - frame.pageXOffset + avg(admin.w, client.w) - 5) + 'px', 
          top:  (client.y - frame.pageYOffset - 4) + 'px'
        });
      });
    }
